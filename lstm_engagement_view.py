...
def run_lstm_engagement_prediction(lstm_model):
    """
    Renders the UI for the LSTM Engagement Prediction feature in the sidebar.
    Takes the already-loaded LSTM model as input.
    """
    st.sidebar.subheader("📈 LSTM Engagement Trend Prediction")
    st.sidebar.info(f"This tool uses the loaded LSTM to predict the 'next day clicks' based on a sequence of {SEQ_LEN} days synthesized from a single 'total_clicks' value.")
    ...
    if uploaded_file:
        try:
            ...
            if "total_clicks" not in df_test.columns:
                st.sidebar.error("❌ 'total_clicks' column missing in uploaded file!")
                return
            
            # --- FIX: Wrap logic in 'with st.sidebar:' ---
            with st.sidebar:
                # --- Place spinner only around heavy computation ---
                ...
                # --- Show results *after* spinner, still in sidebar ---
            ...
            st.sidebar.error(f"Failed during LSTM prediction: {e}")
